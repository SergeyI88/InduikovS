...
*/
	public int[] remove(String[] array) {
		int c = 0; //переменная счетчик количество повторяемых слов
		 	for (int i = 0; i < array.length - с; i++) { //проходим все элементы кроме последнего
				for (int j = i + 1; i < array.length; j++) { // сравнивается отдельно элемент, со всеми после него
					if (array[i] == array[j]) { // сравниваем значение элемента внешнего цикла и внутреннего
						for (int k = j; k < array.length - с; k++) { // если они равны, то запускаем цикл сдвига вправо, тем самым перезаписываем элемент
		...
		return Arrays.copyOf(array, 3); // возвращаем массив обрезанный копиями, которые остались справа
